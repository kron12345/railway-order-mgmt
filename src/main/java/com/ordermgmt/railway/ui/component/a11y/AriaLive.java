...
/**
 * Visually hidden live region for screen-reader announcements. Add once to a view, then call {@link
 * #announce(String)} when something the user should hear changes (selection, filter result count,
 * save success, etc.).
 *
 * <p>{@code aria-live=polite} ensures the announcement is queued without interrupting the current
 * task — appropriate for status changes that are not errors.
 */
