...
| TimingQualifierCode | TimingQualifierCode | TimingQualifierCode | TimingQualifierCode | TTT standard code qualifying a time value: ALA/ALD (exact), ELA/LLA/ELD/LLD (window or half-window), PLA/PLD (commercial). `TimingAtLocation` allows `Timing (0..*)` so single-bound entries (only ELA, only LLD, etc.) are valid — the editor exposes them as AFTER/BEFORE modes |
| TimeConstraintMode | TimeConstraintMode | TimeConstraintMode | TimeConstraintMode | Internal enum with six values mapping to TTT TimingQualifiers: NONE (not exported), EXACT (ALA/ALD), WINDOW (ELA+LLA / ELD+LLD), AFTER (≥, ELA/ELD only), BEFORE (≤, LLA/LLD only), COMMERCIAL (PLA/PLD) |
| userEntered-Flag | userEntered flag | flag userEntered | indicateur userEntered | Per-field boolean on `TimetableRowData` marking whether the user actively typed the value. Acts as the TTT-export gate — only fields with `userEntered = true` produce a `Timing` entry in the Path Request. Mirrored/derived/propagated values stay local |
| Day-Offset (Tag) | Day offset | Offset giorno | Décalage jour | Per-side day-delta from the PlannedCalendar reference (e.g. +1 for next-day on midnight crossings, -1 for modifications starting the day before X). Maps to TTT `Timing/Offset` |
| Halbfenster (TTT) | Half-window | Mezza-finestra | Demi-fenêtre | TTT timing constraint with only one bound (ELA without LLA, or LLD without ELD). Encoded as a single `Timing` entry. Surfaced in the editor as AFTER (≥) and BEFORE (≤) modes |
...
| Fahrplanmanager | Path Manager | Gestore Tracce | Gestionnaire de Sillons | In-app module managing the TTT path request lifecycle — a **mock / stand-in for RailOpt** (the external planning system); used to exercise the timetable-transfer and path-ordering flow without RailOpt |
| RailOpt | RailOpt | RailOpt | RailOpt | QNamic's external railway planning & resource-management product (Fahrplanmanagement, Umlauf-/Ressourcenplanung, TTT/TAF-TAP path ordering). This app creates timetables, transfers them to RailOpt, and monitors the order/path lifecycle for KPIs; the in-app "Fahrplanmanager" is a mock of it |
...
| Umlaufplanung | Vehicle Rotation Planning | Pianificazione Rotazione Veicoli | Planification de Rotation des Véhicules | Planning module that assigns reference trains to physical vehicles (multiple units, locomotives, coach sets) across weekdays, displayed as a Gantt chart |
| TTR (Timetable Redesign) | TTR (Timetable Redesign) | TTR (Ridisegno Orario) | TTR (Refonte de l'Horaire) | European framework for timetable capacity allocation, defining phases from Capacity Strategy (X-60) through Annual/Late/Ad Hoc Ordering to the timetable change date (X) |
| Bestellphase 2 | Ordering Phase 2 (Annual) | Fase di Ordinazione 2 | Phase de Commande 2 | Annual Ordering phase (X-11 to X-8.5 months before timetable start) — full TTT process with draft and final offers, ProcessType = ANNUAL_NEW (0) |
| Bestellphase 3 | Ordering Phase 3 (Late) | Fase di Ordinazione 3 | Phase de Commande 3 | Late Ordering phase (X-8.5 to X-2 months before timetable start) — no draft offers allowed, direct final offer, ProcessType = ANNUAL_LATE (1) |
| Kapazitaetsstrategie | Capacity Strategy | Strategia di Capacità | Stratégie de Capacité | Earliest TTR phase (X-60 to X-36), long-term capacity planning before ordering becomes possible |
| Kapazitaetsmodell | Capacity Model | Modello di Capacità | Modèle de Capacité | TTR phase (X-36 to X-18) for modelling expected capacity needs |
| X-Meilenstein (Fahrplanwechsel) | X-Milestone (Timetable Change) | Milestone X (Cambio Orario) | Jalon X (Changement d'Horaire) | Reference date for TTR phase calculation — the start date of a timetable year (typically mid-December), all phase boundaries are measured as months before X |
| CNA (Capacity Needs Announcement) | CNA (Capacity Needs Announcement) | CNA (Annuncio Necessità di Capacità) | CNA (Annonce des Besoins de Capacité) | Formal announcement of capacity needs in the TTR process, typically submitted during the Capacity Model or Capacity Supply phase |
| Geschäft | Business | Affare | Affaire | Business work item that can be linked many-to-many with order positions and purchase positions; tracks status (IN_BEARBEITUNG, FREIGEGEBEN, …), validity, due date, documents |
| Bedarf | Need | Fabbisogno | Besoin | Resource demand on an order position (`ResourceNeed`) — replaced the older "Ressource" UI label. Coverage is INTERNAL (handled with own personnel/vehicles) or EXTERNAL (fulfilled via PurchasePosition orders) |
| Bestellposition | Purchase Position | Posizione di Acquisto | Poste d'Achat | Concrete purchase order line attached to a `Bedarf` (only for EXTERNAL coverage); for CAPACITY needs always routed through TTT, for VEHICLE/PERSONNEL the choice is internal vs. external |
| Master-Detail-Layout | Master-Detail Layout | Layout Master-Dettaglio | Disposition Maître-Détail | Reusable Vaadin component (`MasterDetailLayout<T>`) backing `/orders` and `/businesses`: scrollable card list left, URL-driven detail panel right, `role="listbox"` semantics with arrow-key navigation |
| Befehlsschnellsuche | Command Palette | Palette Comandi | Palette de Commandes | Keyboard-only fuzzy search (`Ctrl+K`) over orders, businesses, and order positions; Enter activates the highlighted item — Bloomberg-Function-Code-style global lookup |
| Sprunglinks | Skip Links | Link di Salto | Liens d'Évitement | Visually hidden anchors revealed on focus that let keyboard users bypass repetitive landmarks (filter / list / detail), satisfying WCAG 2.4.1 (Bypass Blocks) |
| Spaltenpräferenzen | Column Preferences | Preferenze Colonne | Préférences de Colonnes | Per-user persistence of grid column order, width, and visibility, stored as JSON in `user_view_preferences` keyed by `view_key` (e.g. `grid.business.list`) |
