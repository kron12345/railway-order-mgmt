...
| **Vehicle Planning** | Domain + UI (Gantt Flow) | `VpRotationSet`, `VpVehicle`, `VpRotationEntry`, `VpVehicleOperation`, `Conflict` Record + 2 Enums (`VehicleType`, `CouplingPosition`) | `VehiclePlanningView`, `GanttChart` (tltv Gantt Flow), `ConflictPanel` | Fahrzeugumlaufplanung. V11-Migration (4 Tabellen). Services: VehiclePlanningService + ConflictDetectionService. Gantt Flow Add-on: Shelf-Zeilen (Ablage) + Duty-Zeilen (Dienste), DnD zwischen Zeilen, FPJ-Filter. |
...
| **Datenbank** | V1-V17 Migrationen | Orders, Positionen, Ressourcen, Bestellungen, Infrastruktur, Schlagwoerter, Positionskommentare, Fahrplanarchiv, OTN, Path Manager (8 `pm_*` Tabellen + Audit + Multi-Year Seed + Country-Code-Fix V12/V13), Vehicle Planning (4 `vp_*` Tabellen + OTN V15), TTR-Seed (FPJ 2025/2026/2027), Ressourcenkatalog (V16: `resource_catalog_items` + ResourceNeed/PurchasePosition-Erweiterungen + Seed-Daten), TTT-Bestellattribute (V17: JSONB `ttt_order_attributes`) |
